...
def cancel_hide_wifi_5G(app):
    if SET.set_app(app) ==1:
        if SET.click_wifi(app) ==1:
            if  SET.set_5Gwifi_hide(app,2) == 2:
                return 2

def hide_wifi_5G(app):
    if SET.set_app(app) == 1:
        if SET.click_wifi(app) == 1:
            if  SET.set_5Gwifi_hide(app,1) == 1:
                return 1
...
class Test_retart:
    ...
    def test_cancel_hide_wifi_5G(self):
        assert cancel_hide_wifi_5G(self.app) ==2

    def test_hide_wifi_5G(self):
        assert hide_wifi_5G(self.app) ==1
