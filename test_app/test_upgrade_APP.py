...
def upgrade_cancel(app):
    if SET.set_app(app) ==1:
        if  SET.upgradeRouter_app(app,2) ==2:
            return 2

def upgrade_sure(app):
    if SET.set_app(app) ==1:
        if  SET.upgradeRouter_app(app,1) ==1:
            return 1
...
class Test_retart:
    ...
    def test_restart_cancel(self):
        assert upgrade_cancel(self.app) ==2

    def test_restart_sure(self):
        assert upgrade_sure(self.app) ==1
