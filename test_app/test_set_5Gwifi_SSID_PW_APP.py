...
SSID_5G = config.get('WiFi', 'SSID_5G')
wifi_PW_5G = config.get('WiFi', 'wifi_PW')
logging.info(__file__)

def cancel_set_5Gwifi_SSID_PW_5G(app):
    if SET.set_app(app) ==1:
        if SET.click_wifi(app) ==1:
            if  SET.set_5Gwifi_SSID_PW(app, SSID_5G, wifi_PW_5G, 2) == 2:
                return 2

def set_5Gwifi_SSID_PW_5G(app):
    if SET.set_app(app) == 1:
        if SET.click_wifi(app) == 1:
            if  SET.set_5Gwifi_SSID_PW(app, SSID_5G, wifi_PW_5G, 1) == 1:
                return 1
...
class Test_retart:
    ...
    def test_cancel_set_5Gwifi_SSID_PW_5G(self):
        assert cancel_set_5Gwifi_SSID_PW_5G(self.app) ==2

    def test_set_5Gwifi_SSID_PW_5G(self):
        assert set_5Gwifi_SSID_PW_5G(self.app) ==1
