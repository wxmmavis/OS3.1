                                ...
                                if lr.login(driver, default_pw) == 1:
                                    if rs.setup_choose(driver, 2) == 2:
                                        ...
                                        if rs.getDHCP(driver, dns3, dns4) == 2:
                                            logging.info('=== DHCP Auto Update Success ===')
                                            driver.quit()
                                            return 1
                                        else:
                                            logging.warning('=== DHCP Auto Update Fail ===')
                                            driver.get_screenshot_as_file(projectpath + "/errorpng/caseFail/DHCPautoUpdate-%s.jpg" % time.strftime("%Y%m%d%H%M%S",time.localtime()))
    driver.quit()
